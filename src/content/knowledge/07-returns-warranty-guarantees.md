---
title: 'Returns, warranty, and guarantees'
topic: 'policy'
url: '/terms'
...
questions:
  ...
  - 'Can I return unopened packs?'
  - 'What if the ties do not meet spec?'
...
Any unopened 100-pack, in its original polybag, can be returned within 30 days
of delivery. To start a return:

1. Contact support with the order number and the SKU and quantity being
   returned.
2. A prepaid return label is issued if the return is due to a Shaw Safety error
   (wrong item, wrong quantity, damaged). For a change-of-mind return, the buyer
   covers return shipping.
3. Refund is processed to the original payment method within 5 business days of
   the returned goods arriving at the warehouse.

## Damaged or wrong shipments

If a package arrives damaged, if the wrong SKU was shipped, or if the quantity
is short, contact support within 5 business days of delivery with the order
number and photos of the outer box and the affected ties. A corrected shipment
goes out at no charge and the affected units can be returned at Shaw Safety's
expense or discarded, whichever is simpler.
...
a description or photo of the failure. Replacement is typically dispatched
...
This is deliberate: safety product is often stocked for the season it is
needed, and defects sometimes only surface at use.
...
contact support. The replacement goes out immediately; the carrier claim is
handled internally so that the customer is not caught between the retailer and
the carrier.

## Warranty against manufacturing defect

Shaw Safety warrants that every tie meets the published UL 21S declaration and
the printed spec sheet at the time of shipment. This warranty runs for the
useful shelf life of the product, not just for the return window. Coverage is
limited to replacement of the affected units and does not extend to
consequential damages, lost freight, or downstream operational costs from a
defective lot.

## Not covered by warranty

- Ties that have been installed and cut (installed use is proof of function).
- Ties damaged by improper storage (extreme heat, direct sun for extended
  periods, submersion in solvents).
- Ties used outside their published temperature or chemical resistance range.
- Ties applied to a locking mechanism outside the specified bundle diameter
  range.
...
No restocking fee on standard-stock SKUs returned unopened within 30 days. A
15% restocking fee may apply to unopened returns received between day 31 and
day 60, at Shaw Safety's discretion; after day 60, unopened product is
generally not accepted for return.
